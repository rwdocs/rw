...
- **Mtime caching for site reload** (RD-031 Phase 2); `SiteLoader` caches file modification times and extracted titles between reloads; only files with changed mtimes are re-read for title extraction
- **Skip directory traversal for content edits** (RD-031 Phase 3); `Modified` events now use cached site structure instead of triggering full directory traversal; `Created` events for existing files (common with editors that save via "write temp + rename") also use cached site; reduces live reload latency from ~190ms to ~5ms for content edits; only `Created` events for genuinely new files and `Removed` events trigger full rebuild; ~95% improvement for the common case (content edits)
...
- **`DirectiveProcessor`** for coordinating directive handling; builder pattern with `with_inline()`, `with_leaf()`, `with_container()` methods; `process()` for preprocessing and `post_process()` for HTML transformation; supports recursive markdown expansion with configurable depth limit
- **`DirectiveProcessorConfig`** for processor configuration: `base_dir` for relative path resolution, `source_path` for current file context, `read_file` callback for file system access, `max_include_depth` for recursion limit
- **`TabsDirective`** implementing `ContainerDirective` trait; migrates tabs functionality to new directive API while maintaining full backward compatibility with existing `TabsPreprocessor`/`TabsProcessor`
- **`MarkdownRenderer::with_directives()`** method for integrating directive processing into the rendering pipeline; automatically handles preprocessing and post-processing, and collects warnings into `RenderResult`
...
- **Tightened `rw-diagrams` internal visibility**; `NullCache`, `ImgTagGenerator`, `FigureTagGenerator` changed from `pub` to `pub(crate)`; only types used by other crates remain public (`DiagramProcessor`, `DiagramCache`, `DiagramKey`, `FileCache`, `DiagramOutput`, `DiagramTagGenerator`, `RenderedDiagramInfo`)
