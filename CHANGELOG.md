...
- `PageMeta` now exposes a section-relative `subpath` field (HTTP page API, `@rwdocs/core`, and `@rwdocs/viewer` types) alongside `sectionRef`. Embedding hosts that store their own comments can key them on the stable `(sectionRef, subpath)` pair instead of the URL `path`, so moving or remounting a whole section no longer orphans its comments. See [Embedding](docs/embedding.md).

### Changed

- `rw serve` and the `rw comment` CLI now both key inline and page comments on the stable `(sectionRef, subpath)` pair instead of the page's URL path, so CLI-created and browser-created comments land on the same key. Relocating or remounting a whole section (its `sectionRef` unchanged) no longer orphans the comments on its pages. The `--document` flag on `rw comment add` / `rw comment list` still accepts the URL path; the CLI resolves it to the composite key internally. Comments created in 0.1.25 (keyed by the old path) are not migrated — they remain in the database but are no longer queried, so they effectively disappear from the UI and CLI; negligible impact given 0.1.25's age. A section *rename* or moving a single page *within* a section still changes the key for the affected comments. `--document` now also accepts the markdown source file path with or without the docs-root prefix (e.g. `docs/guide.md` or `guide.md`); the CLI maps it to the page's URL path using the same logic the server uses, so a script or agent can pass the file it just edited directly.
