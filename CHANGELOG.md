...
- **Breaking (`rw-renderer` Rust API):** several changes for downstream crates embedding the renderer — none change `rw` binary behavior:
  - The render surface is consolidated to one entry point, `MarkdownRenderer::render(&self, markdown, pipeline) -> RenderResult`. Code-block and directive processors moved onto a new `Pipeline` type (`with_processor`/`with_directives`); the old events-iterator overload, the `parser_options`/`create_parser` helpers, and `processor_warnings` are gone (warnings now arrive in `RenderResult.warnings`). `CodeBlockProcessor` and `TitleResolver` gained `Send + Sync` bounds.
  - Leaf (`::name`) and container (`:::name` … `:::`) directives are now recognized during the pulldown-cmark event walk and **must be blank-line separated**, so they respect markdown block structure (delimiters inside code/fenced blocks stay literal). `DirectiveProcessor::process` is removed.
  - `DirectiveContext::resolve_path` now returns `Result<PathBuf, ResolveError>` and rejects path-escape inputs (absolute paths, Windows prefixes, `..` above the root, control bytes); the unchecked variant and `resolve_path_safe` are removed.
  - GFM (tables, strikethrough, task lists, `[!NOTE]`-style alerts) is always on; the `with_gfm` builder method is removed.
...
- Minimum supported Rust version raised to 1.96 — downstream crates embedding any `rw-*` crate now need a 1.96+ toolchain.
