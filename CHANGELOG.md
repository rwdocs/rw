...
- `@rwdocs/core` now exports `renderCommentBody(markdown)` — renders a comment body's markdown to the same safe, restricted HTML (`bodyHtml`) that `rw serve` produces, so hosts embedding `@rwdocs/viewer` (e.g. a Backstage backend plugin) that store their own comments can render bodies identically. Raw HTML is escaped, headings are demoted, tables flattened, images dropped, and links keep their `href` only for `http`/`https`/`mailto` schemes (other links render as plain, non-clickable text); blank input renders to an empty string. Returns a `Promise<string>`.
- Deep-link to comments — every comment thread has a shareable `#comment-<id>` URL. Opening such a link scrolls to and reveals the thread: open inline comments activate their article highlight and the comment sidebar, page-level and orphaned comments scroll into the timeline with a brief highlight flash and a persistent tint, and resolved comments auto-expand the "Show resolved" disclosure. Each thread has a "Copy link" button (in `rw serve`); opening an inline thread also reflects it in the address bar (via `replaceState`, so it stays a clean shareable permalink without polluting Back/Forward history). Inbound deep-linking works embedded (e.g. a Backstage host that opens a comment URL), while the copy button is hidden there since the host owns the URL.
- Keyboard navigation between comments — on a page with reviewer comments, press `n` to jump to the next comment and `p` for the previous one; from anywhere on the page the first `n` jumps to the first comment and `p` to the last. Navigation steps through inline (highlighted) comments in document order, then page-level and orphaned comments, and wraps around at the ends. The active comment is scrolled into view (inline comments also open their thread in the margin and darken the highlighted passage; page comments center in the bottom list), and each jump is announced to screen readers. Keys are ignored while typing in a comment box, and modifier combos (e.g. `Cmd+N`) pass through to the browser. Mirrors the comment-stepping shortcuts in Google Docs and Word.
- Live comment refresh — when the `rw comment` CLI (or another browser tab) adds, edits, or resolves a comment while you're viewing that page under `rw serve`, the change now appears without a manual reload. The CLI sends a best-effort, token-authenticated notification (via the `.rw/server.json` runtime-info file) to the running server, which re-broadcasts over the existing live-reload WebSocket; the browser silently refetches the page's comments, preserving any in-progress draft. The CLI stays fully decoupled — if no server is running, the comment is still written and the notify is skipped.
- View resolved comments — the page-comments block gains a minimal "Show resolved" disclosure (with a count badge) that reveals all resolved threads for the page (page-level, orphaned, and inline-anchored), each shown muted with its original-passage quote and a Reopen button. Collapsed by default; the toggle is hidden when a page has no resolved comments. Reopening a thread returns it to the open list (and, for inline comments, restores its article highlight).
- `rw serve` writes a `.rw/server.json` runtime-info file on startup (host, port, pid, version, start time, and a reserved secret token) and removes it on graceful shutdown — including on SIGTERM (`docker stop`, systemd), not just Ctrl-C. The file is written atomically with `0600` permissions and lives in the gitignored `.rw/` directory, so the token never lands in version control. It lets other tooling discover a running server for the project.
