...
- **Standardized error handling on `thiserror`** across all crates; `ConfigError` (`docstage-config`), `RenderError` (`docstage-site`), `DiagramError` and `DiagramErrorKind` (`docstage-diagrams`) now use `thiserror` derive macros instead of manual `Display` and `Error` trait implementations; reduces boilerplate and ensures consistent error chain preservation
- **Standardized error message format** across crates; "IO error" changed to "I/O error" in `ConfigError` (`docstage-config`) and `DiagramErrorKind` (`docstage-diagrams`) for consistency with `RenderError` (`docstage-site`)
