...
## ~~5. Tabs are non-functional~~ FIXED
...
Fixed: `TabsDirective` now supports a `static_mode` that generates radio input +
label HTML instead of button + hidden HTML. CSS `:checked` sibling selectors
toggle panel visibility without JavaScript. `rw techdocs build` enables this mode
automatically. Supports up to 10 tabs per group.
