...
test("every listPages identity resolves to a readable path", async () => {
  await withSite(async (site) => {
    ...
    for (const page of pages) {
      ...
      assert.notEqual(path, null, `${page.sectionRef}#${page.subpath} did not resolve`);
      resolved.push(path);
...
test("resolves a page nested inside a section", async () => {
  await withSite(async (site) => {
    assert.equal(await site.pagePathFor("system:default/payments", "api"), "billing/payments/api");
  });
});
...
test("resolves a page in a non-default namespace", async () => {
  ...
  try {
    ...
    assert.equal(await site.pagePathFor("system:payments/ledger", "api"), "ledger/api");
