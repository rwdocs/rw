//! `docstage confluence update` command implementation.
...
use docstage_core::updater::{DryRunResult, PageUpdater, UpdateConfig, UpdateResult};
...
fn require_confluence_config<'a>(
    ...
    config.confluence.as_ref().ok_or_else(|| {
        output.error("Error: confluence configuration required in docstage.toml");
