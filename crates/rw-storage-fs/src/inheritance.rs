...
mod tests {
    ...
    fn test_merge_title_not_inherited() {
        let parent = Metadata {
            title: Some("Parent Title".to_owned()),
            ..Default::default()
        };
        let child = Metadata::default();
        let merged = merge_metadata(&parent, &child);
    ...
    fn test_merge_title_child_wins() {
        ...
        let merged = merge_metadata(&parent, &child);
        assert_eq!(merged.title, Some("Child Title".to_owned()));
    }

    #[test]
    fn test_merge_description_not_inherited() {
        let parent = Metadata {
            description: Some("Parent description".to_owned()),
            ..Default::default()
        };
        let child = Metadata::default();
        let merged = merge_metadata(&parent, &child);
    ...
    fn test_merge_vars_deep_merged() {
        ...
        parent_vars.insert("key1".to_owned(), serde_json::json!("parent1"));
        parent_vars.insert("key2".to_owned(), serde_json::json!("parent2"));

        let mut child_vars = HashMap::new();
        child_vars.insert("key2".to_owned(), serde_json::json!("child2"));
        child_vars.insert("key3".to_owned(), serde_json::json!("child3"));
